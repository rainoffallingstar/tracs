...
track_extract = function(colData = NULL, loci = NULL, gene = NULL, binsize = 10, nthreads = 1, query_ucsc = TRUE, gtf = NULL, build = "hg38", padding = 0, ideoTblName = "cytoBand",use_conda_env = NULL){
  ...
  trackprep_cmd = Sys.getenv(
    "TRACKTOOLS_TRACKPREP_CMD",
    unset = Sys.getenv("GREATCHIP_TRACKPREP_CMD", unset = "")
  )
  
  .check_windows()
  if(input_bw){
    if(!nzchar(trackprep_cmd)){
  ...
  if(input_bw && nzchar(trackprep_cmd)){
    if(!dir.exists(paths = op_dir)){
      dir.create(path = op_dir, showWarnings = FALSE, recursive = TRUE)
    }
    rust_out = tempfile(pattern = "tracktools_trackprep_", tmpdir = op_dir)
    dir.create(path = rust_out, showWarnings = FALSE, recursive = TRUE)

    input_files = colData$bw_files
    custom_names = colData$bw_sample_names

	    args = c("track-extract",
	             "--out-dir", rust_out,
	             "--binsize", as.character(binsize),
	             "--padding", as.character(padding),
	             "--ideo-tbl", as.character(ideoTblName))

	    # If UCSC is disabled, also disable Rust-side UCSC queries (cytoband + refGene for loci).
	    if(!isTRUE(query_ucsc)){
	      args = c(args, "--no-cytoband")
	      if(is.null(gene)){
	        args = c(args, "--no-gene-models")
	      }else{
	        # For gene mode, gene models can still be provided via --gtf (offline).
	        if(is.null(gtf)){
	          args = c(args, "--no-gene-models")
	        }
	      }
    ...
    if(!is.null(gene)){
      args = c(args, "--gene", as.character(gene), "--build", as.character(build))
      if(!is.null(gtf)){
        args = c(args, "--gtf", as.character(gtf))
      }
    }else{
      args = c(args, "--loci", as.character(loci))
    }

    for(i in 1:length(input_files)){
      args = c(args,
               "--bigwig", as.character(input_files[i]),
               "--sample", as.character(custom_names[i]))
    }

    message("Extracting signals with Rust trackprep: ", trackprep_cmd)
    out = system2(command = trackprep_cmd, args = args, stdout = TRUE, stderr = TRUE)
    status = attr(out, "status")
    if(!is.null(status) && status != 0){
      stop("Rust trackprep failed with status ", status, "\n", paste(out, collapse = "\n"))
    }

    meta_path = file.path(rust_out, "meta.tsv")
    tracks_path = file.path(rust_out, "tracks.tsv")
    if(!file.exists(meta_path)){
      stop("Rust trackprep did not produce meta.tsv\n", paste(out, collapse = "\n"))
    }
    if(!file.exists(tracks_path)){
      stop("Rust trackprep did not produce tracks.tsv\n", paste(out, collapse = "\n"))
    }
    meta = data.table::fread(file = meta_path)
    if(!all(c("key","value") %in% colnames(meta)) && ncol(meta) >= 2){
      colnames(meta)[1:2] = c("key","value")
    }
    loci = meta[key %in% "loci", value]
    if(length(loci) != 1 || is.na(loci[1])){
      stop("Rust trackprep did not produce a single loci in meta.tsv\n", paste(out, collapse = "\n"))
    }
    loci = loci[1]

    tracks = data.table::fread(file = tracks_path)
    req_cols = c("sample","chromosome","start","end","size","max")
    if(!all(req_cols %in% colnames(tracks))){
      stop("Rust trackprep tracks.tsv missing columns: ", paste(setdiff(req_cols, colnames(tracks)), collapse = ", "))
    }
    track_summary = lapply(custom_names, function(nm){
      x = tracks[sample %in% nm, .(chromosome, start, end, size, max)]
      if(all(is.na(x[,max]))){
    ...
    gene_models_path = file.path(rust_out, "gene_models.tsv")
    if(!is.null(gene) && !file.exists(gene_models_path)){
      stop("Rust trackprep did not produce gene_models.tsv for gene extraction\n", paste(out, collapse = "\n"))
    }
    if(file.exists(gene_models_path)){
      etbl = .read_gene_models_tsv(gene_models_path)
    }else{
      etbl = NULL
    }
    cytoband_path = file.path(rust_out, "cytoband.tsv")
    if(file.exists(cytoband_path)){
      cyto = data.table::fread(file = cytoband_path)
    ...
    message("OK!")
    return(list(data = track_summary, colData = colData))
  }
  ...
  if(input_bw){
    ...
    track_summary = .get_summaries(bedSimple = windows, bigWigs = input_files, 
                                   op_dir = op_dir, nthreads = nthreads,use_conda_env = use_conda_env)  
  }else{
...
.get_summaries = function(bedSimple, bigWigs, op_dir = getwd(), nthreads = 1,use_conda_env){
  ...
  summaries = parallel::mclapply(bigWigs, FUN = function(bw){
    ...
    bwtool_cmd = .bwtool_prefix(use_conda_env = use_conda_env)
    cmd = paste(bwtool_cmd, "summary -with-sum -keep-bed -header", bedSimple, bw, paste0(op_dir, bn, ".summary"))
    system(command = cmd, intern = TRUE)
    paste0(op_dir, bn, ".summary")
  }, mc.cores = nthreads)
...
.extract_cytoband = function(chr = NULL, refBuild = "hg19", tblName = "cytoBand"){
  ...
  cmd = paste0(
    "mysql --user=genome --host=genome-euro-mysql.soe.ucsc.edu -A -P 3306 -NAD  ",
    refBuild,
...
.extract_chromHmm_ucsc = function(chr, start, end, refBuild = "hg38", tbl){
  ...
  cmd = paste0("mysql --user=genome --host=genome-euro-mysql.soe.ucsc.edu -A -P 3306 -NAD ",  refBuild,  " -e 'select chrom, chromStart, chromEnd, name from ", tbl, " WHERE chrom =\"", tar_chr, "\"'")
...
.extract_geneModel_ucsc_bySymbol = function(genesymbol, refBuild){
  ...
  cmd = paste0("mysql --user=genome --host=genome-euro-mysql.soe.ucsc.edu -A -P 3306 -NAD ",  refBuild,  " -e 'select chrom, txStart, txEnd, strand, name, name2, exonStarts, exonEnds from refGene WHERE name2 =\"", genesymbol, "\"'")
...
.extract_geneModel_ucsc = function(chr, start = NULL, end = NULL, refBuild = "hg19", txname = NULL, genename = NULL){
  ...
  cmd = paste0("mysql --user=genome --host=genome-euro-mysql.soe.ucsc.edu -A -P 3306 -NAD ",  refBuild,  " -e 'select chrom, txStart, txEnd, strand, name, name2, exonStarts, exonEnds from refGene WHERE chrom =\"", tar_chr, "\"'")
  message(paste0("Extracting gene models from UCSC:\n", "    chromosome: ", tar_chr, "\n", "    build: ", refBuild, "\n    query: ", cmd))
  #system(command = cmd)
  ucsc = data.table::fread(cmd = cmd)
  if(nrow(ucsc) == 0){
...
.make_genome_bed = function(refBuild = "hg19", tss = "start", up = 2500, down = 2500, op_dir = tempdir(), pc_genes = FALSE, for_profile = TRUE){
  ...
  cmd = paste0("mysql --user=genome --host=genome-euro-mysql.soe.ucsc.edu -A -P 3306 -NAD ",  refBuild,  " -e 'select chrom, txStart, txEnd, strand, name, name2 from refGene'")
  message(paste0("Extracting gene models from UCSC:\n", "    build: ", refBuild, "\n    query: ", cmd))
  #system(command = cmd)
  ucsc = data.table::fread(cmd = cmd)
...
.bwt_mats = function(bw, binSize, bed, size, startFrom, op_dir,use_conda_env = NULL){
  ...
  bwtool_cmd = .bwtool_prefix(use_conda_env = use_conda_env)
  if(startFrom == "start"){
    cmd = paste0( bwtool_cmd," matrix -starts -tiled-averages=", binSize, " ", size, " " , bed , " ", bw, " ", paste0(op_dir, "/", bn, ".matrix"))  
  }else if(startFrom == "end"){
    cmd = paste0( bwtool_cmd," matrix -ends -tiled-averages=", binSize, " ", size, " " , bed , " ", bw, " ", paste0(op_dir, "/", bn, ".matrix"))
  }else{
    cmd = paste0( bwtool_cmd," matrix -tiled-averages=", binSize, " ", size, " " , bed , " ", bw, " ", paste0(op_dir, "/", bn, ".matrix"))
  }
  print(cmd)
...
.check_bwtool = function(warn = FALSE, use_conda_env = NULL){
  cmd = .bwtool_prefix(use_conda_env = use_conda_env)
  status = suppressWarnings(system(paste(cmd, "--version"), ignore.stdout = TRUE, ignore.stderr = TRUE))
  check = ifelse(test = status == 0, yes = cmd, no = "")
  if(check != ""){
    if(warn){
...
.bwtool_prefix = function(use_conda_env = NULL){
  cmd = Sys.getenv(
    "TRACKTOOLS_BWTOOL_CMD",
    unset = Sys.getenv("GREATCHIP_BWTOOL_CMD", unset = "")
  )
  if(nzchar(cmd)){
    return(cmd)
  }
  if (!is.null(use_conda_env)){
    return(paste("conda run -n", use_conda_env, "bwtool"))
  }
  return("bwtool")
}

.read_gene_models_tsv = function(path){
  gm = data.table::fread(file = path)
  colnames(gm) = c("chr", "start", "end", "strand", "tx", "gene", "exon_start", "exon_end")
  gm = gm[order(tx, exon_start, exon_end)]
  exon_tbls = lapply(split(gm, as.factor(as.character(gm$tx))), function(x){
    exon_tbl = data.frame(start = as.numeric(as.character(x[,exon_start])),
                          end = as.numeric(as.character(x[,exon_end])))
    attributes(exon_tbl) = list(start = min(x[,start], na.rm = TRUE),
